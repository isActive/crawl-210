...
namespace _210_Crawl
{
    public class RequestXNet
    {
        ...
        public string Post(string url, string data = "", string contentType = "application/x-www-form-urlencoded")
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(data))
                    response = _request.Post(url).ToString();
                else
                    response = _request.Post(url, data, contentType).ToString();
        ...
        public string UpLoad(string url, xNet.MultipartContent data = null) => this._request.Post(url, data)?.ToString();
